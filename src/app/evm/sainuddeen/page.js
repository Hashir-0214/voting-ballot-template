...
export default function CampaignEVM() {
  ...
  const handleVote = (slot) => {
    // 1. Basic check if machine is ready
    if (voteStatus !== "ready") return;

    // 2. CRITICAL CHANGE: Ignore clicks if it is NOT the hero candidate
    // This prevents the LED, Beep, and Confirmation Page for empty slots.
    if (!slot.isHero) return; 

    // 3. Proceed only if it is the correct candidate
  ...
  return (
    <div className="min-h-screen h-full my-auto bg-white flex flex-col items-center justify-center py-8 font-sans select-none">
      <Head>
        <title>Vote for {currentConfig.heroName}</title>
      </Head>
